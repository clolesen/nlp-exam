...
#Run the analysis with k = 150
#RUN THE FUNCTION BLOC BELOW FIRST
Results = RunItAll(150, metadata)
```
...
prepareData = function(metadata){
  ...
  for (id in metadata$ID){
    doc = fread(paste0("data/Processed_1gram/",id,".csv"))
    wVec = c()
...
getSurDiv = function(ATM_list, LDAmodel, metadata, k, skipHist = F){
  ...
  if (skipHist == F) save(mainResults, file = "R_objects/mainResults.Rdata")
...
for (id in subset(Results[[1]], minPrevPub > 1 & nCollab %in% 2:4)$docID){
  ...
  for (x in 1:1){
    ...
    for (name in names){
      ATM = Results[[4]][[name]]
      prev = which(ATM[,151] == id) - 1
      prevTopics = ATM[prev,1:150]
  ...
  print(paste(n,"out of 346 -",346-n,"left", sep = " "))
  n = n + 1
  t = toc()
  t = t$toc[[1]] - t$tic[[1]]
  t_list = c(t_list,t)
...
OTHER STUFF
```{r}
withinSupriseData_list = list()
i = 1
for (author in unique(Results[[2]]$Author)){
  data = Results[[2]][Results[[2]]$Author == author,]
  data = data[data$nColllab < 5,]
  try(if (data[1,"MaxPubNum"] > 2){
    withinSur = mean(data$SurToPrev)
    meanCol = mean(data$nColllab)
    row = data.table(
      Author = author,
      meanCollab = meanCol,
      withinSurprise = withinSur
    )
    withinSupriseData_list[[i]] = row
    i = i + 1
  }, silent = T)
}
withinSupriseData = do.call(rbind, withinSupriseData_list)

ggplot(withinSupriseData, aes(y = meanCollab, x = withinSurprise)) +
  geom_point(size=0.1) +
  geom_smooth()

withinSupriseData[withinSupriseData$meanCollab>15]

Results[[2]][Results[[2]]$Author=="minawestman"]

subset(Results[[1]], Surprise > 3 & nCollab == 3)

metadata[12091,]



ggplot(as.data.frame(Results[[3]]$theta[2,]), aes(y = `Results[[3]]$theta[2, ]`, x = 1:150)) +
  geom_point()

GetTopTerms(Results[[3]]$phi, 10)




# Find lenght of documents
len_list = c()
for (i in 1:17947){
  len = length(str_split(corpus$string[i], " ", simplify = T))
  len_list = c(len_list, len)
}
mean(len_list)
sd(len_list)
```
